...
namespace ConnectCampus.Application.Features.Associations.Commands.UpdateAssociation
{
    public class UpdateAssociationCommandValidator : AbstractValidator<UpdateAssociationCommand>
    {
        ...
        public UpdateAssociationCommandValidator(IAssociationRepository associationRepository)
        {
            RuleFor(x => x.Name)
                ...
                {
                    var existingByName = await associationRepository.GetByIdAsync(cmd.Id, cancellation);
                    if (existingByName != null && existingByName.Name == name)
                    {
                        return true; // Same name as before
                    }
                    return !(await associationRepository.ExistsByNameAsync(name, cancellation));
            ...
            RuleFor(x => x.Slug)
                ...
                .MustAsync(async (cmd, slug, cancellation) =>
                {
                    var existingBySlug = await associationRepository.GetByIdAsync(cmd.Id, cancellation);
                    if (existingBySlug != null && existingBySlug.Slug == slug)
                    {
                        return true; // Same slug as before
                    }
                    return !(await associationRepository.ExistsBySlugAsync(slug, cancellation));
